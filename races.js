        ...
        if (type === "prof") {
          selectionArray = (
            <div key={item}>
              <button
                type="button"
                className="btn btn-light choice"
              ...
              <button
                className="btn badge badge-dark"
                onClick={() => this.removeSelection(type)}
              >
                x
              </button>
  ...
  raceDataHandler(name) {
    if (this.state.toggle === true) {
      const raceInfoRequest = async () => {
                  ...
                  {results.starting_proficiency_options.from.map(
                    (current, i) => (
                      <button
                        ...
                        key={i}
                        onClick={() =>
                          this.addSelection(current.name, "prof", current.url)
                        }
                      >
                        {current.name}
                      </button>
        ...
        //Subraces
        if (results.subraces != "") {
          let subraceUrl = results.subraces.map((current) =>
            current.url.replace("/api/", "")
          );
        ...
        {this.state.toggle && (
          <div className="row">
            {this.props.racesArray.map((current, i) => (
              <div className="col-3 col-md-4 col-lg-1 col-spacing" key={i}>
                <button
